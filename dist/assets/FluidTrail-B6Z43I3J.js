import{r as I,j as $}from"./index-Mu6UxQSY.js";function ee(t){if(!t)return[.4,.4,.4];const s=t.match(/rgba?\(([^)]+)\)/i);if(s){const n=s[1].split(",").map(c=>parseFloat(c));return[(n[0]||0)/255,(n[1]||0)/255,(n[2]||0)/255]}let f=t.replace("#","").trim();f.length===3&&(f=f.split("").map(n=>n+n).join(""));const u=parseInt(f.slice(0,2),16)/255,e=parseInt(f.slice(2,4),16)/255,l=parseInt(f.slice(4,6),16)/255;return[u||0,e||0,l||0]}const j=6e3,te=20,oe=30,re=128,ue=1024,d=`#version 300 es
in vec2 a_pos;
out vec2 v_uv;
void main() {
    v_uv = a_pos * 0.5 + 0.5;
