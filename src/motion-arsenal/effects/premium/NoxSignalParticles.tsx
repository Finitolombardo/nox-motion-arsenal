...
import { clamp, damp, seededRandom, useInView, usePointer, usePrefersReducedMotion } from '../../lib/animationUtils';
...
export function NoxSignalParticles({
  ...
  const mobileBudget = typeof window !== 'undefined' && (window.innerWidth < 720 || window.matchMedia?.('(pointer: coarse)').matches) ? 96 : 180;
  const safeCount = clamp(Math.round(finite(count, 72)), 12, mobileBudget);
  const safeIntensity = clamp(finite(intensity, 1), 0, 1.8);
  const safeSparkSize = clamp(Math.abs(finite(sparkSize, 1.15)), 0.4, 2.8);
  const safeLinkDistance = clamp(Math.abs(finite(linkDistance, 120)), 0, 180);
  const persistence = clamp(finite(trailPersistence ?? trailLength, 0.78), 0, 1);
  const seed = useMemo(() => SIGNAL_PARTICLE_VARIANTS.indexOf(activeVariant) * 173 + 4242, [activeVariant]);
  const signature = `${activeVariant}:${safeCount}:${safeSparkSize}:${seed}`;
  ...
  if (sparkSignature.current !== signature) {
    ...
    sparks.current = Array.from({ length: safeCount }, (_, index) => ({
      ...
      size: (0.58 + random() * 1.15) * safeSparkSize,
  ...
  useCanvas2D(canvasRef, (ctx, size, dt, elapsed) => {
    const { w, h } = size;
    const frameDt = reduced ? 0 : clamp(dt, 1 / 240, 1 / 20);
    const profile = reduced ? 'flow' : activeProfileFor(activeMode, elapsed);
    const profileLabel = reduced ? 'STATIC' : profile.toUpperCase();
    ...
    const burstActive = !reduced && burst.current && burstAge < 1.15;
    ...
    f.px = f.x;
    f.py = f.y;
    f.x = reduced ? targetX : damp(f.x, targetX, 7.5, frameDt);
    f.y = reduced ? targetY : damp(f.y, targetY, 7.5, frameDt);
    const pointerVx = (f.x - f.px) / Math.max(frameDt, 1 / 120);
    const pointerVy = (f.y - f.py) / Math.max(frameDt, 1 / 120);

    // Translucent decay creates a smoke-like flow body without hard line trails.
    ctx.globalCompositeOperation = 'source-over';
    const fade = reduced ? 1 : clamp(0.2 - persistence * 0.145 * energyPreset.persistence, 0.028, 0.2);
    ...
    const attractionRadius = Math.max(120, safeLinkDistance * 2.4);
    const speedLimit = (42 + safeIntensity * 32) * energyPreset.speed;
    ...
    for (let index = 0; index < sparks.current.length; index += 1) {
      ...
      const focusForce = (14 + proximity * 72) * preset.focusBias * energyPreset.focus * profileFocus * safeIntensity;
      const curlForce = (12 + proximity * 46) * preset.curlBias * energyPreset.turbulence * profileCurl * curlPulse * safeIntensity;
      const flowForce = (18 + spark.depth * 18) * energyPreset.turbulence * safeIntensity;
      const burstForce = burstActive ? Math.sin(clamp(burstAge / 1.15, 0, 1) * Math.PI) * 48 : 0;

      if (!reduced) {
        spark.vx += (flowX * flowForce + nx * focusForce * globalPull + tx * curlForce * proximity + nx * burstForce + pointerVx * 0.018 * proximity) * frameDt;
        spark.vy += (flowY * flowForce + ny * focusForce * globalPull + ty * curlForce * proximity + ny * burstForce + pointerVy * 0.018 * proximity) * frameDt;

        const drag = Math.exp(-(1.7 + spark.depth * 0.45) * frameDt);
        spark.vx *= drag;
        spark.vy *= drag;
        ...
        spark.x += spark.vx * frameDt;
        spark.y += spark.vy * frameDt;
        spark.age += frameDt;
    ...
    for (let index = 0; index < sparks.current.length; index += 1) {
      const spark = sparks.current[index];
      const lifeFade = reduced ? 0.82 : clamp(Math.min(spark.age * 1.8, (spark.life - spark.age) * 1.4), 0, 1);
      const flicker = reduced ? 1 : 0.76 + Math.sin(elapsed * (2.2 + spark.depth * 2.6) + spark.phase) * 0.24;
      const alpha = clamp(lifeFade * flicker * (0.28 + spark.depth * 0.5) * safeIntensity * energyPreset.glow, 0, 0.92);
      const radius = spark.size * (0.72 + spark.depth * 0.75);
      const color = index % 3 === 0 ? preset.secondary : preset.primary;
      const expensiveGlow = index % 2 === 0 || spark.depth > 0.72;

      if (expensiveGlow) {
        ctx.shadowColor = rgba(color, alpha * 0.95);
        ctx.shadowBlur = (4 + radius * 4.2) * energyPreset.glow;
      ...
      ctx.shadowBlur = expensiveGlow ? radius * 1.8 : 0;
  ...
  return (
    <div ref={rootRef} className="nsp-root" data-variant={activeVariant} data-energy={activeEnergy} data-reduced={reduced} style={style} onPointerDown={onBurst}>
      <style>{STYLES}</style>
      ...
      <div className="nsp-ui">
        <div className="nsp-stack">
          {showVariantSwitcher ? (
            <div className="nsp-controls" role="group" aria-label="Signal particle variants">
              {SIGNAL_PARTICLE_VARIANTS.map((id) => (
                <button key={id} type="button" className="nsp-btn" data-active={id === activeVariant} aria-pressed={id === activeVariant} onPointerDown={(event: PointerEvent<HTMLButtonElement>) => event.stopPropagation()} onClick={() => setActiveVariant(id)}>
          ...
          {showModeSwitcher ? (
            <div className="nsp-controls" role="group" aria-label="Particle choreography">
              {SIGNAL_PARTICLE_MODES.map((id) => (
                <button key={id} type="button" className="nsp-btn" data-active={id === activeMode} aria-pressed={id === activeMode} onPointerDown={(event: PointerEvent<HTMLButtonElement>) => event.stopPropagation()} onClick={() => setActiveMode(id)}>
                  {id.toUpperCase()}
                </button>
              ))}
            </div>
        ...
        {showEnergySwitcher ? (
          <div className="nsp-controls" role="group" aria-label="Signal particle energy">
            {SIGNAL_PARTICLE_ENERGIES.map((id) => (
              <button key={id} type="button" className="nsp-btn" data-active={id === activeEnergy} aria-pressed={id === activeEnergy} onPointerDown={(event: PointerEvent<HTMLButtonElement>) => event.stopPropagation()} onClick={() => setActiveEnergy(id)}>
      ...
      <div className="nsp-mode" aria-live="polite">MODE // {displayMode}</div>
