...
export const PREMIUM_CATALOG: EffectEntry[] = [
  ...
  },
  {
    meta: {
      ...
      performanceNotes: 'Kontext-Budget 1: EIN WebGL-Canvas wandert zur aktiven Karte (dpr-Cap 1.5). Pointer-Licht, Reflexion, Glare und Caustic-Sweep lassen sich für Produktionsseiten vollständig abschalten; dann läuft kein Lighting-rAF und kein Shader.',
      ...
      description: 'Produktionsreife 3D-Glaskarten mit einem wandernden WebGL-Kontext, optionaler Pointer-Beleuchtung und fünf klar unterscheidbaren Materialsystemen.', importPath: '@/motion-arsenal/effects/premium/NoxGlassCards', usageJsx: '<NoxGlassCards variant="obsidian-caustic-deck" energy="calm" tilt={9} refraction={0.03} fresnel={5.5} gridScale={17} depth={0.7} glassOpacity={0.7} interactiveLighting={false} pointerReflection={false} hoverGlare={false} causticSweep={false} specularIntensity={0.15} ambientReflection={0.12} showVariantSwitcher={false} showEnergySwitcher={false} />',
      props: [
        ...
        { key: 'tilt', label: 'Tilt (°)', type: 'range', default: 9, min: 0, max: 14, step: 0.5 }, { key: 'refraction', label: 'Refraction', type: 'range', default: 0.065, min: 0, max: 0.12, step: 0.005 }, { key: 'fresnel', label: 'Fresnel Power', type: 'range', default: 3.1, min: 1, max: 6, step: 0.1 }, { key: 'gridScale', label: 'Material Scale', type: 'range', default: 15, min: 6, max: 26, step: 1 }, { key: 'depth', label: 'Depth', type: 'range', default: 1, min: 0, max: 1.8, step: 0.1 }, { key: 'glassOpacity', label: 'Glass Opacity', type: 'range', default: 0.86, min: 0.45, max: 1, step: 0.05 }, { key: 'interactiveLighting', label: 'Interactive Lighting', type: 'boolean', default: true }, { key: 'pointerReflection', label: 'Pointer Reflection', type: 'boolean', default: true }, { key: 'hoverGlare', label: 'Hover Glare', type: 'boolean', default: true }, { key: 'causticSweep', label: 'Caustic Sweep', type: 'boolean', default: true }, { key: 'specularIntensity', label: 'Specular Intensity', type: 'range', default: 1, min: 0, max: 1.5, step: 0.05 }, { key: 'ambientReflection', label: 'Ambient Reflection', type: 'range', default: 1, min: 0, max: 1.5, step: 0.05 }, { key: 'showVariantSwitcher', label: 'Variant Switcher', type: 'boolean', default: true }, { key: 'showEnergySwitcher', label: 'Energy Switcher', type: 'boolean', default: true },
